...
func RunningSpentCalories(action int, weight, duration float64) float64 {
	// ваш код здесь
	midSpeed := float64(meanSpeed(action, duration))
	return ((runningCaloriesMeanSpeedMultiplier * midSpeed * runningCaloriesMeanSpeedShift) * weight / float64(mInKm)) * duration * float64(minInH)
}
...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	...
	one := walkingCaloriesWeightMultiplier*weight + (math.Pow(meanSpeedInMsec, 2)/heightInM)*walkingSpeedHeightMultiplier*weight
	return one * duration * minInH

	//((walkingCaloriesWeightMultiplier * weight + ((math.Pow(meanSpeedInMsec, 2) / heightInM) *  walkingSpeedHeightMultiplier * weight) * duration * minInH)
	//((0.035 * ВесСпортсменаВКг + (СредняяСкоростьВМетрахВСекунду**2 / РостВМетрах) * 0.029 * ВесСпортсменаВКг) * ВремяТренировкиВЧасах * minInH)
	// 1. (СредняяСкоростьВМетрахВСекунду**2 / РостВМетрах)
	// 2. (0.035 * ВесСпортсменаВКг + (СредняяСкоростьВМетрахВСекунду**2 / РостВМетрах) * 0.029 * ВесСпортсменаВКг)
	// 3. * ВремяТренировкиВЧасах * minInH)
}
...
func SwimmingSpentCalories(lengthPool, countPool int, duration, weight float64) float64 {
	// ваш код здесь
	midSpeedswim := float64(swimmingMeanSpeed(lengthPool, countPool, duration))
	return (midSpeedswim + swimmingCaloriesMeanSpeedShift) * float64(swimmingCaloriesWeightMultiplier) * weight * duration
}
